...
@admin_bp.route("/api/ollama")
def ollama_status():
    from Config.Settings import OLLAMA_URL, OLLAMA_MODEL, EMBED_MODEL
    try:
        import requests as req
        resp   = req.get(f"{OLLAMA_URL}/api/tags", timeout=3)
        models = [m["name"] for m in resp.json().get("models", [])]
        return jsonify({"online": True, "models": models, "llm": OLLAMA_MODEL, "embed": EMBED_MODEL,
                        "has_llm":   any(OLLAMA_MODEL.split(":")[0] in m for m in models),
                        "has_embed": any(EMBED_MODEL.split(":")[0]  in m for m in models)})
    except Exception:
        return jsonify({"online": False, "llm": OLLAMA_MODEL, "embed": EMBED_MODEL})
