...
from Core.RAG import init_rag, rag_chat
...
def chat():
    ...
    if not init_rag():
        return jsonify({"error": "Ollama nao esta em execucao.", "offline": True}), 503
    try:
        answer, sources = rag_chat(message, history)
        log_action("chat", {"preview": message[:60]})
        return jsonify({"answer": answer, "sources": sources})
    except Exception as e:
        err = str(e)
        if "Connection" in err or "refused" in err.lower():
            return jsonify({"error": "Ollama offline.", "offline": True}), 503
